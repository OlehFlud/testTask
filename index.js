...
async function startRecursive() {
  const ids = [];
  users.splice(0, 10).map(user => ids.push(processUser(user.id)))
  const sum = (await Promise.all(ids)).reduce((accumulator, currentValue) => accumulator + currentValue);
  return users.length ? await startRecursive().then((total) => total + sum) : sum
}
